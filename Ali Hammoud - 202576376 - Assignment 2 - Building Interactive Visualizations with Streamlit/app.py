...
work = df.copy()
if require_bus_stop:
    if bus_stop_col in work.columns:
...
def pick_condition(row):
    # each row has a one-hot among good/acceptable/bad for the selected road type
...
cond_counts = work["Condition"].value_counts(dropna=False).reindex(["Good","Acceptable","Bad","Unknown"]).fillna(0)
...
fig1.update_yaxes(range=[0, max(100, cond_df["% of Towns"].max()+10)])
...
total_mode = mode_df["Towns reporting mode"].sum()
if total_mode > 0:
    mode_df["Share (%)"] = (mode_df["Towns reporting mode"] / total_mode * 100).round(1)
else:
    mode_df["Share (%)"] = 0.0
...
# ---------------------- BONUS INSIGHT: BUS STOPS vs BAD ROADS ----------------------
st.subheader("Quick Diagnostic: Bus Stops vs Bad Roads (selected road type)")

if bus_stop_col in df.columns:
    # compute % bad among towns with and without dedicated bus stops
    tmp = df.copy()
    tmp["Condition"] = tmp.apply(pick_condition, axis=1)
    grp = tmp.groupby(tmp[bus_stop_col].map({1:"With Bus Stops", 0:"No Bus Stops"}))["Condition"].value_counts(normalize=True).rename("%").mul(100).reset_index()
    pivot = grp.pivot(index=bus_stop_col, columns="Condition", values="%").rename_axis("Bus stops").fillna(0)
    pivot = pivot.reindex(["With Bus Stops","No Bus Stops"])
    pivot = pivot[["Bad","Acceptable","Good"]]
