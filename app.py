# app.py — 2x2 layout + heatmap (Governorate×Condition if available, else Mode×Condition)
...
with k1: st.metric("Towns", f"{len(work):,}")
with k2:
...
with k3: st.metric("Road type", road_type.capitalize())
...
# ---------- (1) Condition mix — bar ----------
with top_left:
    st.markdown(f"### Condition mix — {road_type.capitalize()} roads")
...
# ---------- (2) Mode share — donut ----------
with top_right:
    st.markdown("### Main transport mode — share of towns")
...
# ---------- (3) With vs without bus stops — stacked ----------
with bot_left:
    st.markdown("### Do bus stops coincide with better roads?")
...
# ---------- (4) Heatmap ----------
with bot_right:
    # Prefer Governorate × Condition if "Governorate" exists; otherwise Mode × Condition
    if "Governorate" in work.columns and work["Governorate"].notna().any():
        st.markdown("### Heatmap — Governorate × Condition")
        dist = (work
                .dropna(subset=["Governorate"])
                .groupby(["Governorate", "Condition"])
                .size()
                .reset_index(name="towns"))
        pivot = dist.pivot(index="Governorate", columns="Condition", values="towns").fillna(0)
        denom = pivot.sum(axis=1).replace(0, np.nan)
        share = (pivot.div(denom, axis=0) * 100)[["Good", "Acceptable", "Bad"]].fillna(0)
        ...
        st.markdown("### Heatmap — Mode × Condition")
        modes = [m for m in ["taxis", "vans", "buses"] if MODE_COLS[m] in work.columns and m in mode_filter]
        if len(modes) == 0:
            st.info("Select at least one mode to render the heatmap.")
        else:
            rows = []
            for m in modes:
                sub = work[work[MODE_COLS[m]] == 1]
                counts = (sub["Condition"].value_counts(normalize=True) * 100).reindex(["Good","Acceptable","Bad"]).fillna(0)
                rows.append([m.capitalize(), counts.get("Good", 0.0), counts.get("Acceptable", 0.0), counts.get("Bad", 0.0)])
            hm = pd.DataFrame(rows, columns=["Mode","Good","Acceptable","Bad"]).set_index("Mode")

            fig4b = px.imshow(
                hm[["Good","Acceptable","Bad"]],
                labels=dict(x="Condition", y="Mode", color="% of towns"),
                aspect="auto"
            )
