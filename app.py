...
@st.cache_data(show_spinner=False)
def prepare(df: pd.DataFrame, road_type: str, require_bus_stop: bool) -> pd.DataFrame:
    work = df.copy()
    ...
    cond = np.where(work[bad_col].to_numpy() == 1, "Bad", cond)
    cond = np.where((work[bad_col].to_numpy() != 1) & (work[acc_col].to_numpy() == 1), "Acceptable", cond)
    cond = np.where((work[bad_col].to_numpy() != 1) & (work[acc_col].to_numpy() != 1) &
                    (work[good_col].to_numpy() == 1), "Good", cond)
...
total = int(mode_df["Towns reporting mode"].sum()) if not mode_df.empty else 0
mode_df["Share (%)]"] = (mode_df["Towns reporting mode"] / total * 100).round(1) if total else 0.0
...
if BUS_STOP_COL in df.columns:
    ...
    cond = np.where(tmp[b].to_numpy() == 1, "Bad", cond)
    cond = np.where((tmp[b].to_numpy() != 1) & (tmp[a].to_numpy() == 1), "Acceptable", cond)
    cond = np.where((tmp[b].to_numpy() != 1) & (tmp[a].to_numpy() != 1) & (tmp[g].to_numpy() == 1), "Good", cond)
    tmp["Condition"] = cond
